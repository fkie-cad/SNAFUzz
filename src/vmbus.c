...
void sint_post_message(struct context *context, u32 target_vp, u32 synthetic_interrupt_number, u32 event_flag, u32 message_type, void *payload, u8 payload_size){
    ...
    if(context->thread_index != (int)target_vp){
        ...
        ticket_spinlock_lock(&other->pending_message_lock);
        
        if(message_type == /*none*/0 && other->pending_message_send < other->pending_message_reserved && other->pending_messages[(other->pending_message_reserved-1) % array_count(other->pending_messages)].message == null){
            // Reduce the amount of intercore messages...
            other->pending_messages[(other->pending_message_reserved-1) % array_count(other->pending_messages)].event_flag |= event_flag;
        }else{
            struct pending_message *pending_message = &other->pending_messages[other->pending_message_reserved % array_count(other->pending_messages)];
            pending_message->message    = message;
            pending_message->sint_index = synthetic_interrupt_number;
            pending_message->event_flag = event_flag;
            other->pending_message_reserved += 1;
        }
        
        ticket_spinlock_unlock(&other->pending_message_lock);
        return;
    }
    
    ticket_spinlock_lock(&context->pending_message_lock);
    
    u8 *message_page = get_physical_memory_for_write(context, registers->hv_x64_msr_simp & ~0xfff);
    
    volatile struct hv_message *message = (void *)(message_page + 0x100 * synthetic_interrupt_number);
    
    if(message->message_type != /*HvMessageTypeNone*/0){
        message->message_flags |= /*MessagePending*/1;
    }
    
    // It might have changed, between us checking the type and setting the pending bit.
    if(message->message_type == /*HvMessageTypeNone*/0){
        
        if(event_flag){
            u8 *siefp = get_physical_memory_for_write(context, context->registers.hv_x64_msr_siefp & ~0xfff);
            *(u32 *)(siefp + synthetic_interrupt_number * 0x100) |= event_flag;
        }
        
        message->message_type  = message_type;
        message->message_flags = 0;
        message->payload_size  = payload_size;
        memcpy((void *)message->payload, payload, payload_size);
        ...
        context->wait_for_eom = 1;
        
        struct pending_message *pending_message = &context->pending_messages[context->pending_message_reserved % array_count(context->pending_messages)];
        pending_message->message    = (void *)message;
        pending_message->sint_index = synthetic_interrupt_number;
        pending_message->event_flag = event_flag;
        context->pending_message_reserved += 1;
    }
    
    ticket_spinlock_unlock(&context->pending_message_lock);
}
...
void vmbus_send_packet(struct context *context, struct vmbus_channel *channel, struct vmbus_packet_description *packet){
    ...
    *(u32 *)(header + 0) = packet_end;
    ...
    sint_post_message(context, channel->target_vp, /*VMBUS_SINT*/2, 1ull << channel->channel_id, /*HV_MESSAGE_NONE*/0, null, 0);
}
...
void vmbus_process_pending_messages(struct context *context){
    
    ticket_spinlock_lock(&context->pending_message_lock);
    
    if(context->pending_message_send < context->pending_message_reserved){
        
        struct pending_message *pending_message = &context->pending_messages[context->pending_message_send++ % array_count(context->pending_messages)];
        u32 synthetic_interrupt_number = pending_message->sint_index;
        
        if(pending_message->event_flag){
            u8 *siefp = get_physical_memory_for_write(context, context->registers.hv_x64_msr_siefp & ~0xfff);
            *(u32 *)(siefp + synthetic_interrupt_number * 0x100) |= pending_message->event_flag;
        ...
        if(pending_message->message){
            ...
            if(message->message_type == /*none*/0){
                
                memcpy(message, pending_message->message, sizeof(*pending_message->message) + pending_message->message->payload_size);
...
void vmbus_handle_message(struct context *context, u32 connection_id, void *payload, u32 payload_size){
    ...
    switch(message->message_type){
        ...
        case /*VMBUS_MSG_INITIATE_CONTACT*/0xe:{
            ...
            globals.vmbus.monitor_page1 = message->initiate_contact.monitor_page1;
            globals.vmbus.monitor_page2 = message->initiate_contact.monitor_page2;
            globals.vmbus.target_vp = message->initiate_contact.target_vcpu;
            ...
            sint_post_message(context, globals.vmbus.target_vp, /*VMBUS_SINT*/2, /*event_flag*/0, /*HV_MESSAGE_VMBUS*/1, &version_response_message, sizeof(version_response_message));
        }break;
        ...
        case /*VMBUS_MSG_REQUESTOFFERS*/3:{
            ...
            for(u32 at = 0; at < array_count(vmbus_channels); at++){
                ...
                sint_post_message(context, globals.vmbus.target_vp, /*VMBUS_SINT*/2, /*event_flag*/0, /*HV_MESSAGE_VMBUS*/1, &offer_channel_message, sizeof(offer_channel_message) - /*padding*/4);
            ...
            sint_post_message(context, globals.vmbus.target_vp, /*VMBUS_SINT*/2, /*event_flag*/0, /*HV_MESSAGE_VMBUS*/1, &message_type, sizeof(message_type));
        }break;
        ...
        case /*VMBUS_MSG_OPEN_CHANNEL*/5:{
            ...
            if(PRINT_VMBUS_EVENTS || PRINT_VMBUS_INITIALIZATION_EVENTS) print("[" __FUNCTION__ "] Received VMBUS_MSG_OPEN_CHANNEL open id: %d channel id: %d target_vp: %d gpadl id: %d read pages: %d\n", message->open_channel.open_id, message->open_channel.channel_id, message->open_channel.target_vp, message->open_channel.ring_buffer_gpadl_id, message->open_channel.read_ring_buffer_pages);
            
            struct vmbus_channel *channel = globals.vmbus.channels;
            ...
            *(u8 *)(send_buffer_header + 0x40) = 1; 
            ...
            sint_post_message(context, globals.vmbus.target_vp, /*VMBUS_SINT*/2, /*event_flag*/0, /*HV_MESSAGE_VMBUS*/1, &open_channel_result, sizeof(open_channel_result));
        ...
        case /*GPADL_TEARDOWN*/11:{
            ...
            sint_post_message(context, globals.vmbus.target_vp, /*VMBUS_SINT*/2, /*event_flag*/0, /*HV_MESSAGE_VMBUS*/1, &message_gpadl_torndown, sizeof(message_gpadl_torndown));
        }break;
            ...
            case VMBUS_DEVICE_mouse:{
                ...
                switch(synthhid_header->type){
                    case /*SYNTH_HID_PROTOCOL_REQUEST*/0:{
                        ...
                        if(PRINT_VMBUS_INITIALIZATION_EVENTS) print("SYNTH_HID_PROTOCOL_REQUEST version %x %x\n", version->major_version, version->minor_version);
